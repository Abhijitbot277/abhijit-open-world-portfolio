...
function City() {
  ...
  return <group>
    ...
    {Array.from({ length: 12 }).map((_, i) => <mesh key={`dash-${i}`} rotation={[-Math.PI / 2, 0, 0]} position={[(i - 5.5) * 2.2, 0.025, 0]}><planeGeometry args={[1.1, 0.08]} /><meshStandardMaterial color="#d5b96c" emissive="#6b5422" emissiveIntensity={0.4} /></mesh>)}
    {Array.from({ length: 12 }).map((_, i) => <mesh key={`dashz-${i}`} rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.026, (i - 5.5) * 2.2]}><planeGeometry args={[0.08, 1.1]} /><meshStandardMaterial color="#d5b96c" emissive="#6b5422" emissiveIntensity={0.4} /></mesh>)}
    {BUILDINGS.map((b) => <group key={b.label} position={[b.x, b.h / 2, b.z]}>
      ...
      <mesh position={[0, 0, 1.63]}><boxGeometry args={[2.55, b.h * 0.65, 0.025]} /><meshStandardMaterial color="#11151d" metalness={0.6} roughness={0.2} /></mesh>
...
function Car({ active, onEnter }) {
  ...
  return <group ref={ref} position={[0, 0.38, 6.2]} onClick={() => !active && onEnter()}>
    ...
    {[-0.78, 0.78].map((x) => <React.Fragment key={x}><mesh position={[x, 0.05, -0.95]}><cylinderGeometry args={[0.31, 0.31, 0.2, 16]} rotation={[Math.PI / 2, 0, 0]} /><meshStandardMaterial color="#08090b" /></mesh><mesh position={[x, 0.05, 0.95]}><cylinderGeometry args={[0.31, 0.31, 0.2, 16]} rotation={[Math.PI / 2, 0, 0]} /><meshStandardMaterial color="#08090b" /></mesh></React.Fragment>)}
    ...
    {!active && <Text position={[0, 1.2, 0]} fontSize={0.24} color="#f4c86b" anchorX="center">PRESS E TO ENTER</Text>}
  </group>;
}
...
function Player({ activeVehicle, setActiveVehicle, onInteract }) {
  ...
  useEffect(() => { const toggle = (e) => { if (e.key.toLowerCase() === 'e' && activeVehicle) setActiveVehicle(false); }; window.addEventListener('keydown', toggle); return () => window.removeEventListener('keydown', toggle); }, [activeVehicle, setActiveVehicle]);
  useFrame((state, delta) => {
    if (!ref.current || activeVehicle) return;
    const k = keys.current; const input = new THREE.Vector3((k.d || k.arrowright ? 1 : 0) - (k.a || k.arrowleft ? 1 : 0), 0, (k.s || k.arrowdown ? 1 : 0) - (k.w || k.arrowup ? 1 : 0));
    const moving = input.lengthSq() > 0; if (moving) input.normalize();
    const speed = k.shift ? 6.2 : 3.5; velocity.current.lerp(input.multiplyScalar(speed), 1 - Math.pow(0.001, delta));
    ...
    if (leftArm.current) leftArm.current.rotation.x = stride * 0.65; if (rightArm.current) rightArm.current.rotation.x = -stride * 0.65; if (leftLeg.current) leftLeg.current.rotation.x = -stride * 0.7; if (rightLeg.current) rightLeg.current.rotation.x = stride * 0.7;
    if (body.current) body.current.position.y = moving ? Math.abs(Math.sin(state.clock.elapsedTime * (k.shift ? 12 : 9))) * 0.035 : 0;
    const target = ref.current.position.clone().add(new THREE.Vector3(0, 1.15, 0)); const desired = target.clone().add(new THREE.Vector3(5.5, 4.1, 6.5)); camera.position.lerp(desired, 1 - Math.pow(0.002, delta)); camera.lookAt(target);
    let nearest = { b: null, d: Infinity }; BUILDINGS.forEach((b) => { const d = ref.current.position.distanceTo(new THREE.Vector3(b.x, 0, b.z)); if (d < nearest.d) nearest = { b, d }; });
    onInteract(nearest.d < 3.1 ? nearest.b : null);
  });
  return <group ref={ref} position={[0, 0.65, 3]}>
    <group ref={body}><mesh castShadow><capsuleGeometry args={[0.38, 0.85, 6, 12]} /><meshStandardMaterial color="#b08a43" metalness={0.3} roughness={0.45} /></mesh><mesh position={[0, 0.78, 0]} castShadow><sphereGeometry args={[0.34, 16, 16]} /><meshStandardMaterial color="#d8b06a" roughness={0.5} /></mesh><mesh position={[0, 1.1, -0.04]}><boxGeometry args={[0.45, 0.06, 0.08]} /><meshStandardMaterial color="#08090c" /></mesh></group>
    <mesh ref={leftArm} position={[-0.46, 0.18, 0]} castShadow><capsuleGeometry args={[0.11, 0.55, 4, 8]} /><meshStandardMaterial color="#9d783d" /></mesh><mesh ref={rightArm} position={[0.46, 0.18, 0]} castShadow><capsuleGeometry args={[0.11, 0.55, 4, 8]} /><meshStandardMaterial color="#9d783d" /></mesh><mesh ref={leftLeg} position={[-0.18, -0.48, 0]} castShadow><capsuleGeometry args={[0.13, 0.65, 4, 8]} /><meshStandardMaterial color="#5e4b2e" /></mesh><mesh ref={rightLeg} position={[0.18, -0.48, 0]} castShadow><capsuleGeometry args={[0.13, 0.65, 4, 8]} /><meshStandardMaterial color="#5e4b2e" /></mesh>
...
function Game({ setNearby, activeVehicle, setActiveVehicle }) {
  return <Canvas shadows camera={{ position: [7, 5, 9], fov: 50 }}>
    <color attach="background" args={["#08090c"]} /><fog attach="fog" args={["#08090c", 18, 40]} /><ambientLight intensity={0.55} /><directionalLight position={[8, 14, 6]} intensity={2.1} castShadow shadow-mapSize={[2048, 2048]} /><pointLight position={[0, 4, 0]} intensity={8} distance={12} color="#d8a84d" />
    <City /><Car active={activeVehicle} onEnter={() => setActiveVehicle(true)} /><Player activeVehicle={activeVehicle} setActiveVehicle={setActiveVehicle} onInteract={setNearby} /><Environment preset="night" />
  </Canvas>;
}

function App() {
  const [nearby, setNearby] = useState(null); const [panel, setPanel] = useState(null); const [activeVehicle, setActiveVehicle] = useState(false);
  useEffect(() => { const interact = (e) => { if (e.key.toLowerCase() === 'e' && nearby && !activeVehicle) setPanel(nearby); }; window.addEventListener('keydown', interact); return () => window.removeEventListener('keydown', interact); }, [nearby, activeVehicle]);
  return <main>
    <div className="hud"><div className="brand">ABHIJIT CITY</div><div className="mission"><span className="eyebrow">NEW MISSION</span><strong>EXPLORE THE DEVELOPER</strong><small>Walk through the city and discover each district.</small></div><div className="controls"><b>W A S D</b> {activeVehicle ? 'DRIVE' : 'MOVE'} &nbsp; <b>SHIFT</b> {activeVehicle ? 'BOOST' : 'RUN'} &nbsp; <b>E</b> {activeVehicle ? 'EXIT' : 'INTERACT'}</div></div>
    <div className="vehicle-state">{activeVehicle ? '● VEHICLE' : '● ON FOOT'}</div>
    {nearby && !panel && !activeVehicle && <div className="interaction"><span>◈</span> {nearby.label} <b>[E]</b></div>}
    ...
    {activeVehicle && <div className="driving-hint">WASD / ARROWS DRIVE &nbsp; • &nbsp; SHIFT BOOST &nbsp; • &nbsp; E EXIT</div>}
